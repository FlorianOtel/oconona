...
**Behaviour:** Queries OC's DB to fetch parent session and subagent child sessions; reads `agent`, `model`, `cost`, and `tokens_*` directly from the child session rows (populated natively by the OC daemon at Task-tool dispatch time). Computes segment-delta costs (end snapshot − start snapshot) if both snapshots are non-empty.
